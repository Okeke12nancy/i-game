...
const LoginForm = () => {
  ...
  useEffect(() => {
    ...
    if (location.state?.fromRegister) {
      ...
      const timer = setTimeout(() => setSuccessBanner(false), 5000); // Auto-hide after 5s
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (status === 401 || status === 400) {
        // Invalid credentials
        setErrors({
          general: backendMessage || "Invalid username or password.",
        });
      } else {
      ...
      toast.error(backendMessage || "Invalid username or password.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        {successBanner && (
          <div className="bg-green-50 border border-green-200 rounded-md p-3 text-sm text-green-700 text-center">
